...
from data_types import SocTitleMatching, SocJobAdvertMatching, Advert
from datasets import get_soc_dataset
...
def _match(texts: List[str]) -> List[Tuple[str, float]]:
...
async def match_soc_titles(advert: Advert, limit: int = 10):
    ...
    similarities = _match(advert.contents)[:limit]
    titles = [item[0] for item in similarities]
